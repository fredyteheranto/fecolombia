/*
 * Copyright (c) 2011 fredy teheran <fredyteheran91@gmail.com>
 * MIT Licensed
 */
...
Client.prototype._invoke = function (method, args, location, callback, options, extraHeaders) {
  ...
  if ((style === 'rpc') && ((input.parts || input.name === "element") || args === null)) {
    ...
    //console.log('paso 1', message)
  } else {
    assert.ok(!style || style === 'document', 'invalid message definition for rpc style binding');
    // pass `input.$lookupType` if `input.$type` could not be found
    message = self.wsdl.objectToDocumentXML(input.$name, args, input.targetNSAlias, input.targetNamespace, (input.$type || input.$lookupType));
    ////console.log('paso 2', input.$name, args, input.targetNSAlias, input.targetNamespace, (input.$type || input.$lookupType))
  }
  envelopeKey = 'soapenv'
  xml = '<?xml version=\"1.0\" encoding=\"utf-8\"?>' +
  '<' + envelopeKey + ':Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:tem="http://tempuri.org/" xmlns:ser="http://schemas.datacontract.org/2004/07/ServiceSoap.UBL2._0.Models.Object" xmlns:arr="http://schemas.microsoft.com/2003/10/Serialization/Arrays">' +
    //'<' + envelopeKey + ':Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:tem="http://tempuri.org/" xmlns:ser="http://schemas.datacontract.org/2004/07/Services.Models.Object" >' +
